...
input_template_file="template_tmp.yaml"
output_template_file="packaged-template-output.yaml"

cf_stack_name="$project_name"
...
aws cloudformation create-change-set \
    ...
    --parameters ParameterKey="IBComponentUri",ParameterValue="s3://$s3_bucket_deployment/image-builder/components/install-fluentbit.yaml" \
                 ParameterKey="ResourceNamePrefix",ParameterValue=$project_name \
                 ParameterKey="IBImageRecipeParentAmiId",ParameterValue=$parent_image \
                 ParameterKey="MySubnets",ParameterValue="$default_subnets" \
                 ParameterKey="VpcId",ParameterValue="$default_vpc" \
                 ParameterKey="ECRLambdaConsumerRepo",ParameterValue="$ecr_lambda_consumer_repo"
